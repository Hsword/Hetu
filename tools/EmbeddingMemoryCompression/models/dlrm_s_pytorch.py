...
def run():
    ...
    num_fea = len(ln_emb) + 1  # num sparse + num dense features

    m_den_out = ln_bot[len(ln_bot) - 1]
        ...
        if not args.inference_only:
            ...
            while k < args.nepochs:
                ...
                for j, inputBatch in enumerate(train_ld):
                    ...
                    # forward pass
                    # torch.save(dlrm.state_dict(), 'original.bin')
                    # np.save('dense.npy', X.numpy())
                    # np.save('lso.npy', lS_o.numpy())
                    # np.save('lsi.npy', lS_i.numpy())
                    # np.save('label.npy', T.numpy())
                    Z = dlrm_wrap(
                        X,
                        lS_o,
                        lS_i,
                    ...
                    # np.save('loss.npy'.format(j), L)
                    # np.save('inter.npy', dlrm.inter.cpu().detach().numpy())
                    # np.save('bot.npy', dlrm.bot.cpu().detach().numpy())
                    # np.save('embed.npy', dlrm.embed.cpu().detach().numpy())
                    ...
                    with record_function("DLRM backward"):
                        ...
                        optimizer.step()
                        lr_scheduler.step()
                    # torch.save(dlrm.state_dict(), 'trained.bin')
                    # exit()
