...
class BaiChuanEmbeddings_(nn.Module):
...
class BaiChuanLayers_(nn.Module):
...
class BaiChuanPreNorm_(nn.Module):
    def __init__(self, model):
        super().__init__()
        model = model.transformer
...
def construct_sequential_model(model, config):
    ...
    model_.add_module('embeddings', BaiChuanEmbeddings_(model))
    for i in range(config.num_hidden_layers):
        enc = BaiChuanLayers_(model, i, i + 1)
        model_.add_module('layer_%d'%i, enc)
    model_.add_module('prenorm', BaiChuanPreNorm_(model))
    model_.add_module('cls', BaiChuanCls_(model))
    return model_

class BaiChuanModelInfo(ModelInfo):
    def __init__(self, config, args):
        super(BaiChuanModelInfo, self).__init__()
