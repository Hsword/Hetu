...
def parallelism_optimization(args):
    ...
    if hidden_size == 1024:
        ...
        memcost_model_args = {  'parameter_size': parameter_size,
                                ...
                                'other_model_states': 35.5,
                                'other_activation_per_bsz': 10}
        ...
        memcost_model_args = {  'parameter_size': parameter_size,
                                'tp_activation_per_bsz_dict': tp_activation_per_bsz_dict,
                                'other_model_states': 45.56,
                                'other_activation_per_bsz': 13}
        timecost_model_args_with_overlap = { 'parameter_size': parameter_size,
                                'microbatch': True,
                                'optimal_chunk_func': optimal_chunk_func,
                                'sequence_length': 196,
    ...
    def check_cost_model():
        for strategy in strategies:
            re = MemoryCostModel(strategy, global_batch_size=64, **memcost_model_args).get_memory_cost()
            print(form_strategy(strategy), re['enc_total'], re['other'])
        print()
        for strategy in strategies:
            re = TimeCostModel_with_overlap(strategy, global_batch_size=64, **timecost_model_args_with_overlap).gen_result()
