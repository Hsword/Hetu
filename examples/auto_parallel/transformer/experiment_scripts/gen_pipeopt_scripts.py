...
def make_str(fw, model, bs, dv, st, half=False, pix=True, nccl=True):
    ...
    if st in ('optcnn', 'gpipe', 'pipedream', 'pipeopt'):
        ...
        cur_str += ' --save-dir {}large/noover{}{}{}{}'.format(
            model, half_str, st, dv, appen_str)
...
for model, bs in models.items():
    with open('run_{}_large_pipeopt.py'.format(model), 'w') as fw:
        import_os(fw)
        make_dir(fw, model)
        print(file=fw)
        for dv in devices:
...
# run all
with open('run_all_large_pipeopt.py', 'w') as fw:
    for model in models:
        lines = open('run_{}_large_pipeopt.py'.format(model), 'r').readlines()
        fw.writelines(lines)

dv = 16
with open('run_16_all_large_pipeopt.py', 'w') as fw:
    import_os(fw)
    for model, bs in models.items():
        make_dir(fw, model)
        make_allstr(fw, 'pipeopt', model, bs, dv)
