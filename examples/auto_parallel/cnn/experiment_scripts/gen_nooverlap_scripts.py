...
devices = [2, 4, 8]
strategies = ['dp', 'owt', 'optcnn', 'gpipe', 'pipedream']
...
for model, bs in models.items():
    with open('run_{}_noover.py'.format(model), 'w') as fw:
        import_os(fw)
        make_dir(fw, model)
...
# run all
with open('run_all_noover.py', 'w') as fw:
    for model in models:
        lines = open('run_{}_noover.py'.format(model), 'r').readlines()
        fw.writelines(lines)

dv = 16
with open('run_16_all_noover.py', 'w') as fw:
    import_os(fw)
    for model, bs in models.items():
        make_dir(fw, model)
        for st in strategies:
            if model == 'alexnet' and st == 'gpipe':
                continue
            make_allstr(fw, st, model, bs, dv)
        print(file=fw)
