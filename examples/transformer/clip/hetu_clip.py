...
def _expand_mask(mask, input_shape, tgt_len=None):
    ...
    expanded_mask = ht.broadcast_shape_op(
        mask, (bsz, 1, tgt_len, src_len), add_axes=(1, 2))
    inverted_mask = 1.0 - expanded_mask
    inverted_mask = ht.masked_fill_op(
        inverted_mask, inverted_mask, np.finfo(np.float32).min)
...
class CLIPVisionEmbeddings(object):
    def __init__(self, config, name='CLIPVisionEmbeddings'):
        ...
        self.class_embedding = ht.init.random_normal(
            shape=(self.embed_dim, ), name=name+'.class_embedding')
        ...
        self.position_embedding = ht.layers.Embedding(
            self.num_positions, self.embed_dim, name=name+'.position_embedding')
    ...
    def __call__(self, pixel_values, input_shape):
        ...
        patch_embeds = ht.array_reshape_op(
            patch_embeds, (batch_size, self.embed_dim, -1))
        patch_embeds = ht.transpose_op(patch_embeds, (0, 2, 1))

        class_embeds = ht.broadcast_shape_op(
            self.class_embedding, (batch_size, 1, -1))
        embeddings = ht.concat_op(class_embeds, patch_embeds, axis=1)
        embeddings = embeddings + \
            ht.broadcastto_op(self.position_embedding(
                self.position_ids), embeddings)
        return embeddings
...
class CLIPTextEmbeddings(object):
    def __init__(self, config, name='CLIPTextEmbeddings'):
        ...
        self.token_embedding = ht.layers.Embedding(
            config.vocab_size, embed_dim, name=name+'.token_embedding')
        self.position_embedding = ht.layers.Embedding(
            config.max_position_embeddings, embed_dim, name=name+'.position_embedding')
    ...
    def __call__(self, input_ids=None, input_shape=None, position_ids=None, inputs_embeds=None, inputs_embeds_shape=None):
        ...
        if position_ids is None:
            position_ids = ht.slice_op(
                self.position_ids, (0, 0), (-1, seq_length))
        ...
        embeddings = inputs_embeds + \
            ht.broadcastto_op(position_embeddings, inputs_embeds)
...
class CLIPAttention(object):
    def __init__(self, config, name='CLIPAttention'):
        ...
        self.k_proj = ht.layers.Linear(
            self.embed_dim, self.embed_dim, weight_transpose=True, name=name+'.k_proj')
        self.v_proj = ht.layers.Linear(
            self.embed_dim, self.embed_dim, weight_transpose=True, name=name+'.v_proj')
        self.q_proj = ht.layers.Linear(
            self.embed_dim, self.embed_dim, weight_transpose=True, name=name+'.q_proj')
        self.out_proj = ht.layers.Linear(
            self.embed_dim, self.embed_dim, weight_transpose=True, name=name+'.out_proj')

    def _shape(self, tensor, seq_len, bsz):
        tensor = ht.array_reshape_op(
    ...
    def __call__(self, hidden_states, input_shape, attention_mask=None, causal_attention_mask=None, output_attentions=False):
        ...
        if causal_attention_mask is not None:
            attn_weights = ht.array_reshape_op(
        ...
        if output_attentions:
            attn_weights_reshaped = ht.array_reshape_op(
                attn_weights, (bsz, self.num_heads, tgt_len, src_len))
            attn_weights = ht.array_reshape_op(
        ...
        attn_output = ht.array_reshape_op(
...
class CLIPMLP(object):
    def __init__(self, config, name='CLIPMLP'):
        ...
        self.fc1 = ht.layers.Linear(
            config.hidden_size, config.intermediate_size, weight_transpose=True, name=name+'.fc1')
        self.fc2 = ht.layers.Linear(
            config.intermediate_size, config.hidden_size, weight_transpose=True, name=name+'.fc2')
...
class CLIPEncoderLayer(object):
    def __init__(self, config, name='CLIPEncoderLayer'):
        ...
        self.layer_norm1 = ht.layers.LayerNorm(
            self.embed_dim, name=name+'.layer_norm1')
        self.mlp = CLIPMLP(config, name=name+'.mlp')
        self.layer_norm2 = ht.layers.LayerNorm(
            self.embed_dim, name=name+'.layer_norm2')
...
class CLIPEncoder(object):
    def __init__(self, config, name='CLIPEncoder'):
        self.config = config
        self.layers = [CLIPEncoderLayer(
            config, name=name+'.layers.'+str(i)) for i in range(config.num_hidden_layers)]
...
class CLIPTextTransformer(object):
    def __init__(self, config, name='CLIPTextTransformer'):
        ...
        self.final_layer_norm = ht.layers.LayerNorm(
            self.embed_dim, name=name+'.final_layer_norm')
    ...
    def __call__(self, input_ids=None, input_shape=None, attention_mask=None, position_ids=None,
        ...
        hidden_states = self.embeddings(
            input_ids=input_ids, input_shape=input_shape, position_ids=position_ids)
        ...
        pooled_output = ht.slice_by_matrix_op(
            last_hidden_state, index1, index2)
...
class CLIPVisionTransformer(object):
    def __init__(self, config, name='CLIPVisionTransformer'):
        ...
        self.pre_layrnorm = ht.layers.LayerNorm(
            self.embed_dim, name=name+".pre_layrnorm")
        self.encoder = CLIPEncoder(config, name=name+".encoder")
        self.post_layernorm = ht.layers.LayerNorm(
            self.embed_dim, name=name+".post_layernorm")
    ...
    def __call__(
        ...
        encoder_shape = (
            input_shape[0], self.embeddings.num_patches + 1, self.embed_dim)
...
class CLIPModel(object):
    def __init__(self, config, name=''):
        ...
        self.text_model = CLIPTextTransformer(
            text_config, name=name+'.text_model' if name else 'text_model')
        self.vision_model = CLIPVisionTransformer(
            vision_config, name=name+'.vision_model' if name else 'vision_model')

        self.visual_projection = ht.layers.Linear(self.vision_embed_dim, self.projection_dim, bias=False,
                                                  weight_transpose=True, name=name+'.visual_projection' if name else 'visual_projection')
        self.text_projection = ht.layers.Linear(self.text_embed_dim, self.projection_dim, bias=False,
                                                weight_transpose=True, name=name+'.text_projection' if name else 'text_projection')

        self.logit_scale = ht.Variable(
            name=name+'.logit_scale' if name else 'logit_scale', value=np.array([self.config.logit_scale_init_value]))
    ...
    def __call__(self, input_ids, input_ids_shape, pixel_values, pixel_values_shape, attention_mask=None, position_ids=None,
        ...
        logits_per_text = logits_per_text * \
            ht.broadcast_shape_op(logit_scale, (batch_size, batch_size))
