...
class BertEncoder(object):
    def __init__(self, config, layer_id_start = None, layer_num = None):
        ...
        if layer_id_start is not None and layer_num is not None:
            ...
            # if layer_id_start == 12 or layer_id_start == 0:
            #     self.layer = [BertLayer(config) for _ in range(layer_id_start, layer_id_start+layer_num)]
            # else:
            #     self.layer = [BertLayer(config) for _ in range(layer_id_start, layer_id_start+layer_num-3)]
            #     self.layer.append(BertLayer(config, True))
            #     self.layer.append(BertLayer(config, True))
            #     self.layer.append(BertLayer(config, True))
        else:
    ...
    def __call__(self, hidden_states, attention_mask=None):
        ...
        # for i, layer_module in enumerate(self.layer):
        #     attention_output = layer_module.attention(hidden_states, attention_mask)
        #     mini_seq = 64
        #     hidden_states_list = []
        #     for i in range(int(512/mini_seq)):
        #         mini_attention_output = ht.slice_op(attention_output, (0, i*mini_seq, 0), (self.batch_size, mini_seq, self.hidden_size))
        #         intermediate_output = layer_module.intermediate(mini_attention_output) # [bsz, mini_seq, 4*hidden]
        #         hidden_state = layer_module.output(intermediate_output, mini_attention_output) # [bsz, mini_seq, hidden]
        #         hidden_states_list.append(hidden_state) # [bsz, mini_seq, hidden]
        #     hidden_states = ht.concatenate_op(hidden_states_list, axis = 1) # [bsz, seq_len, hidden]
