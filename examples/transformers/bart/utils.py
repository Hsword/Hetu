...
class DataLoader(object):
    ...
    def make_epoch_data(self):
        ...
        for i in range(n_batchs):
            ...
            self.batch_data['X'].append(x)
            self.batch_data['Y'].append(y)
    
    def get_batch(self, idx):
        if idx >= self.batch_num:
            assert False
        self.cur_batch_data['X'] = self.batch_data['X'][idx]
        self.cur_batch_data['Y'] = self.batch_data['Y'][idx]
