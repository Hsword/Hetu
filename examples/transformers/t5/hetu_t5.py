...
class T5DenseActDense(object):
    def __init__(self, config, name='T5DenseActDense'):
        super().__init__()
        self.wi = ht.layers.Linear(config.d_model, config.d_ff, bias=False, weight_transpose=True, name=name+'.wi')
        self.wo = ht.layers.Linear(config.d_ff, config.d_model, bias=False, weight_transpose=True, name=name+'.wo')
    ...
    def __call__(self, hidden_states, input_shape):
        ...
        hidden_states = ht.array_reshape_op(hidden_states, input_shape[:-1]+(self.dim, ))
...
class T5DenseGatedActDense(object):
    def __init__(self, config, name='T5DenseGatedActDense'):
        super().__init__()
        self.wi_0 = ht.layers.Linear(config.d_model, config.d_ff, bias=False, weight_transpose=True, name=name+'.wi_0')
        self.wi_1 = ht.layers.Linear(config.d_model, config.d_ff, bias=False, weight_transpose=True, name=name+'.wi_1')
        self.wo = ht.layers.Linear(config.d_ff, config.d_model, bias=False, weight_transpose=True, name=name+'.wo')
        self.dim = config.d_model        
        self.dropout = ht.layers.DropOut(config.dropout_rate)
        if config.dense_act_fn == "relu":
...
class T5LayerFF(object):
    def __init__(self, config, name='T5LayerFF'):
        ...
        if config.is_gated_act:
            self.DenseReluDense = T5DenseGatedActDense(config, name=name+'.DenseReluDense')
        else:
            self.DenseReluDense = T5DenseActDense(config, name=name+'.DenseReluDense')

        self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon, name=name+'.layer_norm')
        self.dropout = ht.layers.DropOut(config.dropout_rate)
...
class T5Attention(object):
    def __init__(self, config, has_relative_attention_bias=False, name='T5Attention'):
        ...
        self.q = ht.layers.Linear(self.d_model, self.inner_dim, bias=False, weight_transpose=True, name=name+'.q')
        self.k = ht.layers.Linear(self.d_model, self.inner_dim, bias=False, weight_transpose=True, name=name+'.k')
        self.v = ht.layers.Linear(self.d_model, self.inner_dim, bias=False, weight_transpose=True, name=name+'.v')
        self.o = ht.layers.Linear(self.inner_dim, self.d_model, bias=False, weight_transpose=True, name=name+'.o')

        if self.has_relative_attention_bias:
            self.relative_attention_bias = ht.layers.Embedding(self.relative_attention_num_buckets, self.n_heads, name=name+'.relative_attention_bias')
    ...
    def _relative_position_bucket(relative_position, bidirectional=True, num_buckets=32, max_distance=128):
        ...
        if bidirectional:
            ...
            relative_position = (-1) * ht.min_op(relative_position, ht.zeroslike_op(relative_position))
        ...
        relative_position_if_large = max_exact + (
            ht.log_op(relative_position * (1/max_exact)) *(1/ math.log(max_distance / max_exact))* (num_buckets - max_exact)
        )
        relative_position_if_large = ht.min_op(
            relative_position_if_large, ht.full_like_op(relative_position_if_large, num_buckets - 1)
        )
        if(relative_buckets):
            relative_buckets = relative_buckets + ht.where_op(is_small, relative_position, relative_position_if_large)
        else:
            relative_buckets = ht.where_op(is_small, relative_position, relative_position_if_large)
        return relative_buckets

    def compute_bias(self, batch_size, query_length, key_length):
        context_position = ht.arange_op(0, query_length)
        context_position = ht.broadcast_shape_op(context_position, (query_length, key_length))
        memory_position = ht.arange_op(0, key_length)
        memory_position = ht.broadcast_shape_op(memory_position, (query_length, key_length))
        ...
        values = ht.broadcast_shape_op(values, (batch_size, self.n_heads, query_length, key_length), add_axes=(0))
    ...
    def __call__(
        ...
        key_length = real_seq_length if key_value_states is None else key_value_states_shape[1]

        def shape(states):
            states = ht.array_reshape_op(states, (batch_size, -1, self.n_heads, self.key_value_proj_dim))
        ...
        def project(hidden_states, proj_layer, key_value_states, past_key_value):
            ...
            if past_key_value is not None:
                if key_value_states is None:
                    hidden_states = ht.concat_op(past_key_value, hidden_states, axis=2)
                else:
        ...
        if(key_value_states):
            key_value_states = ht.array_reshape_op(key_value_states, (-1, input_shape[-1]))
        ...
        if position_bias is None:
            if not self.has_relative_attention_bias:
                position_bias = ht.init.zeros((batch_size, self.n_heads, real_seq_length, key_length))
            else:
                position_bias = self.compute_bias(batch_size, real_seq_length, key_length)

            if past_key_value is not None:
                position_bias = ht.slice_op(position_bias, (0, 0, -input_shape[1], 0), (-1, -1, -1, -1))
            if mask is not None:
...
class T5LayerSelfAttention(object):
    def __init__(self, config, has_relative_attention_bias=False, name='T5LayerSelfAttention'):
        super().__init__()
        self.SelfAttention = T5Attention(config, has_relative_attention_bias=has_relative_attention_bias, name=name+'.SelfAttention')
        self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon, name=name+'.layer_norm')
        self.dropout = ht.layers.DropOut(config.dropout_rate)
...
class T5LayerCrossAttention(object):
    def __init__(self, config, name='T5LayerCrossAttention'):
        super().__init__()
        self.EncDecAttention = T5Attention(config, has_relative_attention_bias=False, name=name+'.EncDecAttention')
        self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon, name=name+'.layer_norm')
        self.dropout = ht.layers.DropOut(config.dropout_rate)
...
class T5Block(object):
    def __init__(self, config, has_relative_attention_bias=False, name='T5Block'):
        super().__init__()
        self.is_decoder = config.is_decoder
        self.layer = [T5LayerSelfAttention(config, has_relative_attention_bias=has_relative_attention_bias, name=name+'.layer.0')]
        if self.is_decoder:
            self.layer.append(T5LayerCrossAttention(config, name=name+'.layer.1'))
...
class T5Stack(object):
    def __init__(self, config, embed_tokens=None, name='T5Stack'):
        ...
        self.block = [T5Block(config, has_relative_attention_bias=bool(i == 0), name=name+'.block.'+str(i)) for i in range(config.num_layers)]
        
        self.final_layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon, name=name+'.final_layer_norm')
        self.dropout = ht.layers.DropOut(config.dropout_rate)
    ...
    def create_extended_attention_mask_for_decoder(self, input_shape, attention_mask, attention_mask_shape):
        ...
        seq_ids_a = ht.broadcast_shape_op(seq_ids, (batch_size, seq_length, seq_length), add_axes=(0, 1))
        seq_ids_b = ht.broadcast_shape_op(seq_ids, (batch_size, seq_length, seq_length), add_axes=(0, 2))
        ...
        if seq_length < attention_mask_shape[1]:
            ...
            causal_mask = ht.concat_op(ht.init.ones((batch_size, seq_length, prefix_seq_len), trainable=False), causal_mask, axis=-1)

        causal_mask = ht.array_reshape_op(causal_mask, (batch_size, 1, seq_length, seq_length))
        attention_mask = ht.broadcast_shape_op(attention_mask, (batch_size, 1, seq_length, seq_length), add_axes=(1, 2))
    ...
    def __call__(
        ...
        mask_seq_length =past_key_values_shape[0][0][2] + seq_length if past_key_values is not None else seq_length

        if use_cache is True:
            assert self.is_decoder, f"`use_cache` can only be set to `True` if {self} is used as a decoder"

        if attention_mask is None:
            attention_mask = ht.init.ones((batch_size, mask_seq_length), trainable=False)
        if self.is_decoder and encoder_attention_mask is None and encoder_hidden_states is not None:
            encoder_seq_length = encoder_hidden_states_shape[1]
            encoder_attention_mask = ht.init.ones((batch_size, encoder_seq_length), trainable=False)

        if past_key_values is None:
            past_key_values = [None] * len(self.block)

        extended_attention_mask = self.get_extended_attention_mask(attention_mask, (batch_size, mask_seq_length), input_shape)
...
class T5Model(object):
    def __init__(self, config):
        ...
        self.config = config
        self.dim = config.d_model
        self.shared = ht.layers.Embedding(config.vocab_size, config.d_model, name='shared')
...
class T5ForPretraining(object):
    ...
    def _shift_right(self, input_ids, input_shape):
        ...
        shifted_input_ids = ht.init.zeros(input_shape, trainable=False)
        ...
        mask = ht.bool_op(shifted_input_ids, -100)
        shifted_input_ids = ht.masked_fill_op(shifted_input_ids, mask, pad_token_id)
        return shifted_input_ids
        
    def __call__(self, input_ids, input_shape, attention_mask=None, labels=None):
        hidden_states = self.transformer(input_ids, input_shape, attention_mask=attention_mask, decoder_input_ids=self._shift_right(input_ids, input_shape), decoder_input_shape=input_shape)[0]
