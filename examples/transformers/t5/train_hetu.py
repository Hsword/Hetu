...
import logging
import hetu as ht
from hetu_t5 import T5ForPretraining
...
def pretrain(args):        
    ...
    config = T5Config(vocab_size=args.vocab_size, 
                        d_model=args.hidden_size,
                        num_layers=args.num_hidden_layers, 
                        num_heads=args.num_attention_heads, 
                        d_ff=args.hidden_size*4, 
                        dropout_rate=args.dropout_prob,
                        feed_forward_proj=args.hidden_act,
    ...
    loss, lm_logits = model(input_ids, input_shape, attention_mask=attention_mask, labels=labels)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--hidden_size", type=int, default=512, help="Hidden size of transformer model",
    )
    parser.add_argument(
