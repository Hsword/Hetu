...
def tokenize(text, model='t5-small', add_eos=False):
    if not os.path.exists('vocab.json'):
        origin = ht.tokenizers.T5Tokenizer.pretrained_vocab_files_map["vocab_file"][model]
        print('Downloading vocab from %s' % origin)
        urllib.request.urlretrieve(origin, "vocab.json") 
    tokenizer = ht.tokenizers.T5Tokenizer('vocab.json')
