...
class GPT2MLP(object):
    def __init__(self, intermediate_size, config, name='GPT2MLP'):
        ...
        if config.activation_function == "relu":
            self.act = ht.relu_op
        elif config.activation_function == "gelu" or config.activation_function == "gelu_new" :
            self.act = ht.gelu_op
...
class GPT2Model(object):
    ...
    def get_input_embeddings(self):
        return self.wte.embedding_table
    ...
    def __call__(self, input_ids=None, input_shape=None, past_key_values=None, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None,
        ...
        hidden_states = inputs_embeds + ht.broadcastto_op(position_embeds, inputs_embeds)
...
class GPT2LMHeadModel(object):
    def __init__(self, config):
        ...
        self.lm_head = ht.layers.Linear(config.n_embd, config.vocab_size, weight_transpose=True, bias=False)

    def get_input_embeddings(self):
        return self.transformer.get_input_embeddings()
                    
    def __call__(self, input_ids, input_shape, token_type_ids=None, attention_mask=None, labels=None):
        hidden_states = self.transformer(input_ids, input_shape, token_type_ids=token_type_ids, attention_mask=attention_mask)
        ...
        if labels is not None:
            shift_logits = ht.slice_op(lm_logits, (0, 0, 0), (-1, seq_len-1, -1))
            shift_labels = ht.slice_op(labels, (0, 1), (-1, -1))
            loss = ht.crossentropy_sparse_op(ht.softmax_op(shift_logits), shift_labels)
            return loss, lm_logits
        else:
            return lm_logits


class GPT2ForSequenceClassification(object):
    def __init__(self, config):
        self.model = GPT2Model(config)
        self.n_embd = config.n_embd
        self.score = ht.layers.Linear(config.n_embd, config.num_labels, weight_transpose=True, bias=False, name='score')

    def get_input_embeddings(self):
        return self.model.get_input_embeddings()
                        
    def __call__(self, input_ids, input_ids_shape, attention_mask=None, labels=None):
        hidden_states = self.model(input_ids, input_ids_shape, attention_mask=attention_mask)
        
        index1 = ht.arange_op(0, input_ids_shape[0])
        index2 = ht.ne_op(input_ids, 20259)
