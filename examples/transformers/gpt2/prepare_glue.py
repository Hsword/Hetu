...
def get_tokenizer(model='gpt2', add_tokens=None):
    if not os.path.exists('vocab.json'):
        origin = ht.tokenizers.GPT2Tokenizer.pretrained_vocab_files_map["vocab_file"][model]
        print('Downloading vocab from %s' % origin)
        urllib.request.urlretrieve(origin, "vocab.json")
    if not os.path.exists('merges.txt'):
        origin = ht.tokenizers.GPT2Tokenizer.pretrained_vocab_files_map["merges_file"][model]
        print('Downloading merges from %s' % origin)
        urllib.request.urlretrieve(origin, "merges.txt")        
    tokenizer = ht.tokenizers.GPT2Tokenizer('vocab.json', 'merges.txt', bos_token = '<|start|>', eos_token = '<|end|>', pad_token = '<pad>')
