...
class TransfoXLModel(object):
    ...
    def get_input_embeddings(self):
        return self.word_emb.emb_layers
...
class TransfoXLForPretraining(object):
    def __init__(self, config):
...
class TransfoXLForSequenceClassification(object):
    def __init__(self, config):
        self.config = config
        self.model = TransfoXLModel(config)
        self.hidden_size = config.d_model
        self.score = ht.layers.Linear(config.d_embed, config.num_labels, weight_transpose=True, bias=False, name='score')
