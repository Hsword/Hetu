...
from hetu_transfoxl import TransfoXLForSequenceClassification
from transfoxl_config import TransfoXLConfig
...
def get_tokenize(add_tokens=None):
    if not os.path.exists('vocab.pkl'):
        origin = ht.tokenizers.TransfoXLTokenizer.pretrained_vocab_files_map["pretrained_vocab_file"]['transfo-xl-wt103']
        print('Downloading vocab from %s' % origin)
        urllib.request.urlretrieve(origin, "vocab.pkl")

    tokenizer = ht.tokenizers.TransfoXLTokenizer(pretrained_vocab_file='vocab.pkl', bos_token='<bos>', pad_token='<pad>')   
...
def finetune(args):
    ...
    tokenizer = get_tokenize(add_tokens)
    vocab_size = args.vocab_size
    new_num_tokens = len(tokenizer)
            
    config = TransfoXLConfig(vocab_size=new_num_tokens, 
                    d_model=args.hidden_size,
                    d_embed=args.hidden_size,
                    n_layer=args.num_hidden_layers, 
                    n_head=args.num_attention_heads, 
                    d_head=args.hidden_size//args.num_attention_heads,
    ...
    if not os.path.exists('pytorch_model.bin'):
        origin = "https://huggingface.co/transfo-xl-wt103/resolve/main/pytorch_model.bin"
    ...
    for node in executor.param_nodes:
        if node == embedding_node[-1].embedding_table and vocab_size!=new_num_tokens:
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--vocab_size", type=int, default=267735, help="Total number of vocab"
    )
    parser.add_argument(
        "--hidden_size", type=int, default=1024, help="Hidden size of transformer model",
    )
    parser.add_argument(
        "--num_hidden_layers", type=int, default=18, help="Number of layers"
    )
    parser.add_argument(
        "-a",
        "--num_attention_heads",
