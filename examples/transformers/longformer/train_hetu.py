...
from hetu_longformer import LongformerForMaskedLM
...
def pretrain(args):        
    ...
    config = LongformerConfig(vocab_size=args.vocab_size, 
    ...
    train_file = os.path.join(data_dir, 'train.h5py')   
    ...
    loss, lm_logits = model(input_ids, input_shape, attention_mask=attention_mask, attention_mask_shape=input_shape, labels=labels)
    ...
    for ep in range(num_epochs):
        step_num = 0
        dataloader = DataLoader(train_file, args.batch_size)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--gpu_id', type=int, default=1, help='Id of GPU to run.'
    )
    parser.add_argument(
