...
import logging
import hetu as ht
from hetu_longformer import LongformerForSequenceClassification
...
def finetune(args):
    ...
    config = LongformerConfig(vocab_size=new_num_tokens, 
    ...
    loss, logits = model(input_ids, (batch_size, seq_len), attention_mask, (batch_size, seq_len), label_ids)
    ...
    if not os.path.exists('pytorch_model.bin'):
        origin = "https://huggingface.co/allenai/longformer-base-4096/resolve/main/pytorch_model.bin"
    ...
    for node in executor.param_nodes:
        if node == embedding_node and vocab_size!=new_num_tokens:
            value = model_dict['longformer.'+node.name]
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--max_position_embeddings", type=int, default=4098, help="Maximum sequence len"
    )
