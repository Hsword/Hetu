...
import os
import hetu as ht
from hetu_bigbird import BigBirdForPreTraining
from bigbird_config import BigBirdConfig
from utils import DataLoaderForPretraining
import numpy as np
import time
import argparse
...
def pretrain(args):
    ...
    config = BigBirdConfig(vocab_size=args.vocab_size, 
    ...
    _,_, masked_lm_loss, next_sentence_loss = model(input_ids, input_shape, token_type_ids, masked_lm_labels=masked_lm_labels, next_sentence_label=next_sentence_label)
    ...
    for ep in range(num_epochs):
        ...
        for train_file in train_files:
            dataloader = DataLoaderForPretraining(train_file, args.batch_size, pred_len)
