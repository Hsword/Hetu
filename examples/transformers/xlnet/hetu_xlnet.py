...
class XLNetRelativeAttention(object):
    def __init__(self, config, name='XLNetRelativeAttention'):
        ...
        self.q = ht.init.random_normal((config.d_model, self.n_head, self.d_head), name=name+'.q')
        self.k = ht.init.random_normal((config.d_model, self.n_head, self.d_head), name=name+'.k')
        self.v = ht.init.random_normal((config.d_model, self.n_head, self.d_head), name=name+'.v')
        self.o = ht.init.random_normal((config.d_model, self.n_head, self.d_head), name=name+'.o')
        self.r = ht.init.random_normal((config.d_model, self.n_head, self.d_head), name=name+'.r')

        self.r_r_bias = ht.init.random_normal((self.n_head, self.d_head), name=name+'.r_r_bias')
        self.r_s_bias = ht.init.random_normal((self.n_head, self.d_head), name=name+'.r_s_bias')
        self.r_w_bias = ht.init.random_normal((self.n_head, self.d_head), name=name+'.r_w_bias')
        self.seg_embed = ht.init.random_normal((2, self.n_head, self.d_head), name=name+'.seg_embed')

        self.layer_norm = ht.layers.LayerNorm(config.d_model, eps=config.layer_norm_eps, name=name+'.layer_norm')
    ...
    def rel_shift(x, x_shape, klen=-1):
        x_size = x_shape
        x = ht.array_reshape_op(x, (x_size[1], x_size[0], x_size[2], x_size[3]))
        x = ht.slice_op(x, (1, 0, 0, 0), (-1, -1, -1, -1))
        x = ht.array_reshape_op(x, (x_size[0], x_size[1] - 1, x_size[2], x_size[3]))
    ...
    def rel_shift_bnij(x, x_shape, klen=-1):
        ...
        x = ht.array_reshape_op(x, (x_size[0], x_size[1], x_size[3], x_size[2]))
        x = ht.slice_op(x, (0, 0, 1, 0), (-1, -1, -1, -1))
        x = ht.array_reshape_op(x, (x_size[0], x_size[1], x_size[2], x_size[3] - 1))
    ...
    def rel_attn_core(
        ...
        bd_shape = (q_head_shape[1], q_head_shape[2], q_head_shape[0], k_head_r_shape[0])
    ...
    def post_attention(self, h, attn_vec, attn_vec_shape, residual=True):
        ...
        attn_vec = ht.array_reshape_op(attn_vec, (-1, attn_vec_shape[2] * attn_vec_shape[3]))
        attn_out = ht.matmul_op(attn_vec, o)
        attn_out = ht.array_reshape_op(attn_out, (attn_vec_shape[0], attn_vec_shape[1], -1))
    ...
    def __call__(
        ...
        if g is not None:
            ...
            k_head_h = ht.matmul_op(cat, ht.array_reshape_op(self.k, (h_shape[-1], -1)))
            k_head_h = ht.array_reshape_op(k_head_h, (h_shape[0], h_shape[1], self.n_head, self.d_head))

            v_head_h = ht.matmul_op(cat, ht.array_reshape_op(self.v, (h_shape[-1], -1)))
            v_head_h = ht.array_reshape_op(v_head_h, (h_shape[0], h_shape[1], self.n_head, self.d_head))

            r = ht.array_reshape_op(r, (-1, r_shape[-1]))
            sr = ht.array_reshape_op(self.r, (r_shape[-1], -1))
            k_head_r = ht.matmul_op(r, sr)
            k_head_r = ht.array_reshape_op(k_head_r, (r_shape[0], r_shape[1], self.n_head, self.d_head))

            q_head_h = ht.matmul_op(ht.array_reshape_op(h, (-1, h_shape[-1])), ht.array_reshape_op(self.q, (h_shape[-1], -1)))
            q_head_h = ht.array_reshape_op(q_head_h, (h_shape[0], h_shape[1], self.n_head, self.d_head))
            ...
            q_head_g = ht.matmul_op(g, q)
            q_head_g = ht.array_reshape_op(q_head_g, (g_shape[0], g_shape[1], self.n_head, self.d_head))


            if target_mapping is not None:
                q_head_g = ht.transpose_op(q_head_g, (1, 3, 2, 0))
                target_mapping = ht.transpose_op(target_mapping, (2, 0, 1))
                target_mapping = ht.broadcast_shape_op(target_mapping, (-1, self.d_head, -1, -1), add_axes=(1, ))
                ...
                attn_vec_g = self.rel_attn_core(
                    ...
                    v_head_h,
                    k_head_r,
                    (target_mapping_shape[1], g_shape[1], self.n_head, self.d_head),
                    (h_shape[0], h_shape[1], self.n_head, self.d_head),
                    (r_shape[0], r_shape[1], self.n_head, self.d_head),
                    seg_mat=seg_mat,
                ...
                target_mapping = ht.broadcast_shape_op(target_mapping, (-1, self.n_head, -1, -1), add_axes=(1, ))
            ...
            output_h = self.post_attention(h, attn_vec, (h_shape[0], h_shape[1], self.n_head, self.d_head))
            output_g = None
...
class XLNetFeedForward(object):
    def __init__(self, config, name='XLNetFeedForward'):
        ...
        self.layer_norm = ht.layers.LayerNorm(config.d_model, eps=config.layer_norm_eps, name=name+'.layer_norm')
        self.layer_1 = ht.layers.Linear(config.d_model, config.d_inner, weight_transpose=True, name=name+'.layer_1')
        self.layer_2 = ht.layers.Linear(config.d_inner, config.d_model, weight_transpose=True, name=name+'.layer_2')
        self.dropout = ht.layers.DropOut(config.dropout)
...
class XLNetModel(object):
    def __init__(self, config):
        ...
        self.word_embedding = ht.layers.Embedding(config.vocab_size, config.d_model, name='word_embedding')
        self.mask_emb = ht.init.random_normal((1, 1, config.d_model), name='mask_emb')
        self.layer = [XLNetLayer(config, name='layer.'+str(i)) for i in range(config.n_layer)]
        self.dropout = ht.layers.DropOut(config.dropout)
    ...
    def positional_embedding(pos_seq, inv_freq, pos_seq_len, bsz=None):
        ...
        pos_emb = ht.concat_op(ht.sin_op(sinusoid_inp), ht.cos_op(sinusoid_inp), axis=-1)

        if bsz is not None:
            pos_emb = ht.broadcast_shape_op(pos_emb, (-1, bsz, -1), add_axes=(1, ))
        else:
    ...
    def relative_positional_encoding(self, qlen, klen, bsz=None):
        ...
        if self.bi_data:
            ...
            if self.clamp_len > 0:
                fwd_pos_seq = ht.clamp_op(fwd_pos_seq, min=-self.clamp_len, max=self.clamp_len)
                bwd_pos_seq = ht.clamp_op(bwd_pos_seq, min=-self.clamp_len, max=self.clamp_len)

            if bsz is not None:
                fwd_pos_emb = self.positional_embedding(fwd_pos_seq, inv_freq, fwd_pos_seq_len, bsz // 2)
                bwd_pos_emb = self.positional_embedding(bwd_pos_seq, inv_freq, bwd_pos_seq_len, bsz // 2)
                pos_emb_shape = (fwd_pos_seq_len, bsz, self.d_model)
            else:
                fwd_pos_emb = self.positional_embedding(fwd_pos_seq, inv_freq, fwd_pos_seq_len)
                bwd_pos_emb = self.positional_embedding(bwd_pos_seq, inv_freq, bwd_pos_seq_len)
            ...
            if self.clamp_len > 0:
                fwd_pos_seq = ht.clamp_op(fwd_pos_seq, min=-self.clamp_len, max=self.clamp_len)
            pos_emb = self.positional_embedding(fwd_pos_seq, inv_freq, pos_seq_len, bsz)
            pos_emb_shape = (pos_seq_len, bsz if bsz is not None else 2, self.d_model)
    ...
    def __call__(
        ...
        **kwargs
        ...
        token_type_ids = ht.transpose_op(token_type_ids, (1, 0)) if token_type_ids is not None else None
        input_mask = ht.transpose_op(input_mask, (1, 0)) if input_mask is not None else None
        attention_mask = ht.transpose_op(attention_mask, (1, 0)) if attention_mask is not None else None
        perm_mask = ht.transpose_op(perm_mask, (1, 2, 0)) if perm_mask is not None else None
        target_mapping = ht.transpose_op(target_mapping, (1, 2, 0)) if target_mapping is not None else None        
        ...
        if self.attn_type == "uni":
            ...
            attn_mask = ht.broadcast_shape_op(attn_mask, (-1, -1, 1, 1), add_axes=(2, 3))
        ...
        if data_mask is not None:
            if mlen > 0:
                mems_mask = ht.init.zeros([data_mask_shape[0], mlen, bsz], trainable=False)
                data_mask = ht.concat_op(mems_mask, data_mask, axis=1)
            if attn_mask is None:
                attn_mask = ht.broadcast_shape_op(data_mask, (-1, -1, -1, 1), add_axes=(3, ))
            else:
                data_mask = ht.broadcast_shape_op(data_mask, (-1, -1, -1, 1), add_axes=(3, ))
                attn_mask += data_mask
        ...
        if attn_mask is not None:
            ...
            if mlen > 0:
                non_tgt_mask = ht.concat_op(ht.init.zeros([qlen, mlen], trainable=False), non_tgt_mask, axis=-1)
            non_tgt_mask = ht.broadcast_shape_op(data_mask, (-1, -1, 1, 1), add_axes=(2, 3))
        ...
        if target_mapping is not None:
            word_emb_q = ht.broadcast_shape_op(self.mask_emb, (target_mapping_shape[0], bsz, -1))
        ...
        pos_emb, pos_emb_shape = self.relative_positional_encoding(qlen, klen, bsz=bsz)
        pos_emb = self.dropout(pos_emb)


        if head_mask is not None:
            if len(head_mask_shape) == 1:
                head_mask = ht.broadcast_shape(head_mask, (self.n_layer, 1, 1, 1, -1), add_axes=(0, 1, 2, 3))

            elif len(head_mask_shape) == 2:
                head_mask = ht.broadcast_shape(head_mask, (-1, 1, 1, 1, -1), add_axes=(1, 2, 3))
        else:
...
class XLNetForPretraining(object):
    def __init__(self, config):
        self.transformer = XLNetModel(config)
        self.n_embd = config.d_model
        self.lm_head = ht.layers.Linear(config.d_model, config.vocab_size, weight_transpose=True, bias=False)

    def __call__(self, input_ids, input_shape, labels):
        hidden_states = self.transformer(input_ids, input_shape)[0]
...
class XLNetForSequenceClassification(object):
    def __init__(self, config):
        self.config = config
        self.model = XLNetModel(config)
        self.hidden_size = config.d_model
        self.summary = ht.layers.Linear(config.d_model, config.d_model, weight_transpose=True, name='summary')
        self.activation = ht.tanh_op
        self.last_dropout = ht.layers.DropOut(config.dropout)
        self.logits_proj = ht.layers.Linear(config.d_model, config.num_labels, weight_transpose=True, name='logits_proj')

    def get_input_embeddings(self):
        return self.model.get_input_embeddings()
                        
    def __call__(self, input_ids, input_ids_shape, labels=None):
        hidden_states = self.model(input_ids, input_ids_shape)[0]
        hidden_states = ht.slice_op(hidden_states, (0, -1, 0), (-1, 1, -1))
        hidden_states = ht.array_reshape_op(hidden_states, (-1, self.hidden_size))
        hidden_states = self.summary(hidden_states)
        hidden_states = self.activation(hidden_states)
        hidden_states = self.last_dropout(hidden_states)
        logits = self.logits_proj(hidden_states)
