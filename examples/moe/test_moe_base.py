...
def moe(args, x, y_, device_id):
    ...
    gate = htl.BalanceAssignmentGate(embed_dim = args.model_dim, num_tokens = args.batch_size * args.num_tokens, \
                        num_experts = args.num_local_experts * num_gpus, device_id=device_id)

    model = htl.MoELayer(gate = gate, experts = experts, num_tokens = args.num_tokens, embed_dim = args.model_dim, all2all_size=num_gpus, name = 'BalanceAssignmentLayer', device_id=device_id)
...
if __name__ == "__main__":
    ...
    loss, y = moe(args, x, y_, device_id)
