...
class RobeLookUp(Op):
    def __init__(self, roar, index, len, x, Z, MO, ctx=None):
    ...
    def compute(self, input_vals, output_val, stream_handle=None):
        if self.on_cpu:
            ...
            robe_lookup(input_vals[0], input_vals[1], input_vals[2], output_val, self.len, self.Bg, self.Cg, self.Dg, self.Z, self.MO, stream_handle)


    def gradient(self, output_grad):
        self.grad_node = robe_lookup_gradient_op(
            output_grad, self.inputs[1], self.inputs[2], None, self.Bg, self.Cg, self.Dg, self.Z, self.MO, ctx=self.raw_ctx)
...
class RobeLookUp_Gradient(Op):
    def __init__(self, vectors, index, x, robe_shape, Bg, Cg, Dg, Z, MO, ctx=None):
        inputs = [vectors]
    ...
    def compute(self, input_vals, output_val, stream_handle=None):
        ...
        if self.index is None:
            output_val.update(
                values=input_vals[0], indices=input_vals[1], x = input_vals[2], Bg = self.Bg, Cg = self.Cg, Dg = self.Dg, Z = self.Z, MO=self.MO, dense_shape=self.robe_shape)
        else:
            assert(0)
            output_val.update(
                values=input_vals[0], indices=self.index, x = self.x, Bg = self.Bg, Cg = self.Cg, Dg = self.Dg, Z = self.Z, MO=self.MO, dense_shape=self.robe_shape)
...
def robe_lookup_op(roar, index, len, x, Z, MO, ctx=None):
    ...
    """
    return RobeLookUp(roar, index, len, x, Z, MO, ctx=ctx)


def robe_lookup_gradient_op(vectors, index, x, robe_shape, Bg, Cg, Dg, Z,MO, ctx=None):
    ...
    """
    return RobeLookUp_Gradient(vectors, index, x, robe_shape, Bg, Cg, Dg, Z,MO, ctx=ctx)
