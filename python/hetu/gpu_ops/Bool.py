...
class BoolValOp(Op):
    ...
    def compute(self, input_vals, output_val, stream_handle=None):
        if self.on_cpu:
            ...
            bool_val(input_vals[0], output_val,
                     self.val, self.cond, stream_handle)
    ...
    def compute(self, input_vals, output_val, stream_handle=None):
        if self.on_cpu:
            ...
            bool_matrix(input_vals[0], input_vals[1],
                        output_val, self.cond, stream_handle)
...
def bool_op(node_A, node_B=None, cond=0, ctx=None):
    """Boolean Node.
    ...
    """
    if (node_B is None):
        return BoolOp(node_A, ctx=ctx)
    if (isinstance(node_B, Op)):
        return BoolMatrixOp(node_A, node_B, cond=cond, ctx=ctx)
    return BoolValOp(node_A, node_B, cond=cond, ctx=ctx)
