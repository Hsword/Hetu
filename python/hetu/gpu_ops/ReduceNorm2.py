...
import numpy as np
from .Node import Op
from ..gpu_links import reduce_norm2


class ReduceNorm2Op(Op):
    def __init__(self, node_A, axes, keepdims=False, ctx=None):
        super().__init__(ReduceNorm2Op, [node_A], ctx)
        self.grad_node = None
        if axes is not None:
        ...
        if self.naive_copy:
            ...
            if self.on_cpu:
                if all(self.keepdims) or not any(self.keepdims):
                    output_val[:] = np.linalg.norm(input_vals[0].asnumpy(), axis=tuple(
                ...
                reduce_norm2(input_vals[0], output_val,
    ...
    def gradient(self, output_grad):
        ...
        self.grad_node = broadcast_shape_op(
            div_op(output_grad, self, ctx=self.raw_ctx), None, None, ctx=self.raw_ctx)
        return [mul_op(self.inputs[0], self.grad_node, ctx=self.raw_ctx)]
...
def reduce_norm2_op(node, axes, keepdims=False, ctx=None):
