...
__global__ void max_kernel(float *input, float *output_val, int ROW, int COL) {
...
int DLGpuMax(const DLArrayHandle input, DLArrayHandle output_val, int dim,
    ...
    if (stream_handle) {
        max_kernel<<<blocks, threads, 0,
                     *(cudaStream_t *)stream_handle->handle>>>(
            input_data, output_val_data, ROW, COL);
    } else {
        max_kernel<<<blocks, threads>>>(input_data, output_val_data, ROW, COL);
    }

    return 0;
}

__global__ void max_mat_kernel(float *matA, float *matB, float *output,
                               size_t size) {
...
int DLGpuMaxMat(const DLArrayHandle matA, const DLArrayHandle matB,
    ...
    if (stream_handle)
        max_mat_kernel<<<blocks, threads, 0,
                         *(cudaStream_t *)stream_handle->handle>>>(
            matA_data, matB_data, output_data, size);
    else
        max_mat_kernel<<<blocks, threads>>>(matA_data, matB_data, output_data,
                                            size);
