...
__global__ void ele_multiply_kernel(const float *matA, const float *matB,
...
__global__ void ele_broadcast_multiply_kernel(const float *matL, const float *matS,
                                         float *output, size_t size,
    ...
    for (int i = 0; i < L_ndims; ++i) {
        uint multiplyer = temp / L_strides[i];
        if (L_ndims - i <= S_ndims && S_dims[i - (L_ndims - S_ndims)] > 1) {
            s_ind += S_strides[i - (L_ndims - S_ndims)] * multiplyer;
        }
        temp %= L_strides[i];
    }
...
__global__ void ele_lazy_multiply_kernel(const float *matA, index_t *matA_stride,
    ...
    if (lazy_input) {
        ...
        if (cu_stream != NULL) {
            ...
            CUDA_CALL(cudaMemcpyAsync(output_stride, output->stride,
                                      ndim * sizeof(index_t),
                                      cudaMemcpyHostToDevice, cu_stream));
            ele_lazy_multiply_kernel<<<blocks, threads, 0, cu_stream>>>(
            ...
            CUDA_CALL(cudaMemcpy(output_stride, output->stride,
                                 ndim * sizeof(index_t),
                                 cudaMemcpyHostToDevice));
            ele_lazy_multiply_kernel<<<blocks, threads>>>(
                matA_data, matA_stride, matB_data, matB_stride, output_data,
                output_stride, size, ndim);
        }
    } else {
        if (size_A == size_B) {
            if (cu_stream != NULL)
                ele_multiply_kernel<<<blocks, threads, 0, cu_stream>>>(
                    matA_data, matB_data, output_data, size);
            else
                ele_multiply_kernel<<<blocks, threads>>>(matA_data, matB_data,
                                                    output_data, size);
        } else {
            ...
            if (cu_stream != NULL) {
                ...
                CUDA_CALL(cudaMemcpyAsync(gpu_dimsB, B_dims, allocatedB,
                                          cudaMemcpyHostToDevice, cu_stream));
                if (size_A > size_B) {
                    ele_broadcast_multiply_kernel<<<blocks, threads, 0, cu_stream>>>(
                ...
                CUDA_CALL(cudaMemcpy(gpu_dimsB, B_dims, allocatedB,
                                     cudaMemcpyHostToDevice));
                if (size_A > size_B) {
                    ele_broadcast_multiply_kernel<<<blocks, threads>>>(
                        matA_data, matB_data, output_data, size, gpu_stridesA,
                        gpu_stridesB, gpu_dimsA, gpu_dimsB, (size_t)matA->ndim,
                        (size_t)matB->ndim);
                } else {
...
/* below is the simple version of multiply elementwise */

__global__ void ele_lazy_multiply_kernel_simple(const float *matA, const float *matB,
                                           float *output,
...
int DLGpuMatrixElementwiseMultiplySimple(const DLArrayHandle matA,
    ...
    if (stream_handle) {
        ...
        ele_multiply_kernel<<<blocks, threads, 0, cu_stream>>>(matA_data, matB_data,
                                                          output_data, size);
    } else {
        ele_multiply_kernel<<<blocks, threads>>>(matA_data, matB_data, output_data,
                                            size);
    }
    return 0;
}
...
int DLGpuMatrixElementwiseMultiplyLazy(const DLArrayHandle matA,
    ...
    if (stream_handle) {
        cudaStream_t cu_stream = *(cudaStream_t *)(stream_handle->handle);
        ele_lazy_multiply_kernel_simple<<<blocks, threads, 0, cu_stream>>>(
            matA_data, matB_data, output_data, gpu_buffer, size, ndim);
    } else {
        ele_lazy_multiply_kernel_simple<<<blocks, threads>>>(
            matA_data, matB_data, output_data, gpu_buffer, size, ndim);
    }
    return 0;
}
